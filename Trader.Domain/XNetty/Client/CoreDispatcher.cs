...
namespace Abp.VNext.Hello.XNetty
{
    public class CoreDispatcher
    {
       ...
       // public Dictionary<string, object> Settings { get; set; } = SqLiteUtils.Settings();
        ...
        private readonly ConcurrentDictionary<Tuple<int, int>, Action<string>> HandlerActions = new ConcurrentDictionary<Tuple<int, int>, Action<string>>();
        ...
        {
            Parallel.ForEach(Account.Actions, (item) => HandlerActions.TryAdd(item.Key, item.Value));
        ...
        public void ExecAction(object message)
        {
            ...
            Tuple<int, int> actionKey = Tuple.Create(reply.Scope, reply.Cmd);
        ...
        public Task ShutdownGracefullyAsyn(Action whenShutdown)
        {
            try
            {
                ...
                return Task.Run(() =>
                {
                    ...
                    ClientBootstrap.Client.ChatChannel.CloseAsync();
